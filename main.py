#Ý TƯỞNG
#THÊM NÚT CHỌN ẢNH, XỬ LÝ ẢNH, XOAY ẢNH, PHÓNG TO, THU NHỎ, LẬT ẢNH
...
class ImageProcessingApp:
    ...
    def create_widgets(self):
        ...
        self.flip_button = tk.Button(self.root, text="LẬT ẢNH", command=self.flip_image)
    ...
    def flip_image(self):
        if self.original_image is None:
            messagebox.showwarning("CẢNH BÁO", "HÃY CHỌN ẢNH TRƯỚC KHI LẬT ẢNH.")
            return

        flipped_image = cv2.flip(self.original_image, 1)  # Lật ảnh theo chiều ngang
        self.original_image = flipped_image
        self.display_image(self.original_image)
